...
import {
  ...
  Quiz as QuizType,
  useQuizgeek,
} from "@/providers/QuizgeekProvider";
import { Button } from "@/components/ui/button";
import { useTheme } from "@/providers/ThemeProvider";
import { Quiz } from "./Quiz";
import ArticleSummary from "./ArticleSummary";
import { ListContent } from "./_articleListed";
import {
  Select,
  SelectContent,
  SelectItem,
  ...
  if (mode === "intermission") {
    return (
      <div className="p-10 rounded flex flex-col items-center">
        ...
        >
          <Button
            className={`w-1/3 m-1 rounded-2xl ${theme === "dark" ? "bg-slate-700 text-slate-50" : "bg-slate-100 text-slate-700"} inset-shadow-sm ${tab === "article" ? "inset-shadow-slate-500/50" : "shadow-sm shadow-slate-500/50"} `}
            onClick={() => {
              setTab("article");
            }}
          >
          ...
          <Button
            className={`w-1/3 m-1 rounded-2xl ${theme === "dark" ? "bg-slate-700 text-slate-50" : "bg-slate-100 text-slate-700"} inset-shadow-sm ${tab === "new" ? "inset-shadow-slate-500/50" : "shadow-sm shadow-slate-500/50"} `}
            onClick={() => {
              setTab("new");
            }}
          >
          ...
          <Button
            className={`w-1/3 m-1 rounded-2xl ${theme === "dark" ? "bg-slate-700 text-slate-50" : "bg-slate-100 text-slate-700"} inset-shadow-sm ${tab === "saved" ? "inset-shadow-slate-500/50" : "shadow-sm shadow-slate-500/50"} `}
            onClick={() => {
              setTab("saved");
            }}
          >
        ...
        <div className={`w-full p-5 ${tab === "saved" && "hidden"}`}>
          ...
          <div className="flex gap-3 mx-2">
            ...
            <Input
              ...
              placeholder="Optional API key. Visit OpenAI API platform to get your custom API key"
              onChange={(e) => {
                setOptions((prev) => ({
                  ...prev,
        ...
        {tab === "saved" && (
          <div className="w-full h-full flex flex-col gap-2 p-5">
            ...
            <div className="mt-4 p-4 rounded-lg bg-slate-900 text-slate-100">
              {view
                ? savedStatus || "Select an article to check saved quiz."
                : "Select an article to verify saved quiz status."}
            </div>
          </div>
        )}
      </div>
